		...
		this._prepare_grid = function (obj) {
			...
			defaultWidth = gs.columnWidth, conf = gs.defaultConf, cellClickHandler = function (tree,node,val,col,t) {
				return function(e) {
					...
					if($(e.target)[0].tagName !="INPUT") //if event trigger from input ignore it can be detected on class/id if want
					tree.trigger("select_cell.jstree-grid", [{_this:this,t:t, col:col, mtarget:e.target, value: val,column: col.header,node: node,grid:$(this),sourceName: col.value}]);
				};
